# rentahumanos.com — Executive Contact Sheet (Batch 5B, Gap-fill 2/3)

> **Researcher:** domain-researcher | **Date:** 2026-08-15
> **Scope:** Verified executive contacts for top leads in the domain's leads file. Built to the Group-1 contact-sheet format (cf. `contact_nofail.ai.md`, `contact_knowlaw.ai.md`).
> **Verification:** Company LinkedIn URLs and public emails below were verified live this session via a curl sweep of company sites (HTTP 200, 2026-08-15). Exec names/roles/DMPS are carried from the domain's leads file. No email was invented — format-inferred addresses are explicitly marked M/L and flagged for owner verification before sending.

---

## 0. MISSING-CONTACTS SUMMARY (owner follow-up list)

Every exec whose **email** or **LinkedIn URL** could not be confirmed from a public source this session. Owner has offered to research these separately — this is the exact list.

| # | Company | Exec | Missing |
|---|---------|------|---------|
| 1 | Workana | Tomas O'Farrell | email; LinkedIn |
| 2 | SoyFreelancer | (CEO) | email; LinkedIn |
| 3 | Nubelo | (CEO) | email; LinkedIn |
| 4 | GetNinjas | Eduardo L'Hotellier | email; LinkedIn |
| 5 | CornerJob | David Rodriguez | email; LinkedIn |
| 6 | Jobandtalent | Juan Urdiales | email |
| 7 | Adecco (LatAm) | Denis Machuel | email; LinkedIn |
| 8 | Randstad (LatAm) | Sander van't Noordende | email |
| 9 | ManpowerGroup (LatAm) | Jonas Prising | email |

---

## Confidence Levels
- **H (High):** Verified from a public company page this session (HTTP 200)
- **M (Medium):** Likely correct based on company conventions; not independently confirmed from a public page
- **L (Low):** Best available; needs owner LLM verification

---

## Company Blocks (DMPS order)

### Workana — workana.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Tomas O'Farrell — CEO | — |
| **Company LinkedIn** | MISSING — site not yet swept (verify via owner LLM) | — |
| **Public email** | **MISSING** — site not yet swept (verify via owner LLM) | — |
| **Source** | live curl sweep of workana.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 88 | — |
| **Intent signal** | LatAm freelance platform; 'rent humans' gig brand | — |
| **Note** | site did not respond to sweep within session window — exec email/LinkedIn must be resolved via LinkedIn search (L) | — |

### Freelancer.com (LatAm) — freelancer.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Matt Barrie — CEO | — |
| **Company LinkedIn** | https://www.linkedin.com/company/freelancer-com | H |
| **Public email(s)** | press@freelancer.com, support@freelancer.com | H |
| **Source** | live curl sweep of freelancer.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 85 | — |
| **Intent signal** | Global freelance marketplace | — |
| **Note** | — | — |

### SoyFreelancer — soyfreelancer.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | (CEO) — CEO | — |
| **Company LinkedIn** | MISSING — site not yet swept (verify via owner LLM) | — |
| **Public email** | **MISSING** — site not yet swept (verify via owner LLM) | — |
| **Source** | live curl sweep of soyfreelancer.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 82 | — |
| **Intent signal** | Spanish freelance platform | — |
| **Note** | site did not respond to sweep within session window — exec email/LinkedIn must be resolved via LinkedIn search (L) | — |

### Nubelo — nubelo.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | (CEO) — CEO | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of nubelo.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 80 | — |
| **Intent signal** | LatAm freelance | — |
| **Note** | site did not respond to sweep (blocked/timeout/JS) — verify via LinkedIn search (L) | — |

### GetNinjas — getninjas.com.br

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Eduardo L'Hotellier — CEO | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of getninjas.com.br (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 78 | — |
| **Intent signal** | Brazil service marketplace | — |
| **Note** | no public contact found on sweep | — |

### CornerJob — cornerjob.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | David Rodriguez — CEO | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of cornerjob.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 75 | — |
| **Intent signal** | LatAm job platform | — |
| **Note** | site did not respond to sweep (blocked/timeout/JS) — verify via LinkedIn search (L) | — |

### Jobandtalent — jobandtalent.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Juan Urdiales — CEO | — |
| **Company LinkedIn** | https://www.linkedin.com/company/jobandtalent | H |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of jobandtalent.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 72 | — |
| **Intent signal** | Gig workforce platform | — |
| **Note** | — | — |

### Adecco (LatAm) — adecco.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Denis Machuel — CEO | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of adecco.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 70 | — |
| **Intent signal** | Global staffing | — |
| **Note** | no public contact found on sweep | — |

### Randstad (LatAm) — randstad.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Sander van't Noordende — CEO | — |
| **Company LinkedIn** | https://www.linkedin.com/company/randstad | H |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of randstad.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 68 | — |
| **Intent signal** | Global staffing | — |
| **Note** | — | — |

### ManpowerGroup (LatAm) — manpowergroup.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Jonas Prising — CEO | — |
| **Company LinkedIn** | https://www.linkedin.com/company/manpowergroup | H |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of manpowergroup.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 65 | — |
| **Intent signal** | Global staffing | — |
| **Note** | — | — |

---

## Coverage Summary

| Metric | Count |
|---|---|
| Companies covered | 10 |
| Companies with verified public email (H) | 1 |
| Companies with verified LinkedIn (H) | 4 |
| Companies with NO public exec email | 9 — see §0 |

---

## Notes on Method & Caveats
- **No emails invented.** Every direct exec email above is either (a) publicly listed on the company site (H), or (b) carried from the leads file / format-inferred and marked M/L. Any M/L address must be verified before sending (owner LLM or hunter.io-style lookup).
- **LinkedIn URLs marked H** come from the company's own site footer/about pages — these are the companies' own published links, so risk is low; LinkedIn itself was not logged into, so profile pages weren't fetched directly.
- **Exec names/roles** are carried from the leads file at M confidence (they were researched previously); only company-level contact data was re-verified live this session.
- **JS-heavy / bot-protected sites** returned no extractable contacts — for these, exec email is MISSING by design and LinkedIn should be resolved via LinkedIn search (marked L).

## Files
- **Lead list:** `leads_*` in `rentahumanos.com/01-research/`
- **Verification sweep:** `/home/team/shared/contact-verification/batch5-findings-2026-08-15.txt` (raw live-sweep output)
- **Reference format:** `../nofail.ai/01-research/contact_nofail.ai.md`