# topaiguys.com — Executive Contact Sheet (Batch 5B, Gap-fill 2/3)

> **Researcher:** domain-researcher | **Date:** 2026-08-15
> **Scope:** Verified executive contacts for top leads in the domain's leads file. Built to the Group-1 contact-sheet format (cf. `contact_nofail.ai.md`, `contact_knowlaw.ai.md`).
> **Verification:** Company LinkedIn URLs and public emails below were verified live this session via a curl sweep of company sites (HTTP 200, 2026-08-15). Exec names/roles/DMPS are carried from the domain's leads file. No email was invented — format-inferred addresses are explicitly marked M/L and flagged for owner verification before sending.

---

## 0. MISSING-CONTACTS SUMMARY (owner follow-up list)

Every exec whose **email** or **LinkedIn URL** could not be confirmed from a public source this session. Owner has offered to research these separately — this is the exact list.

| # | Company | Exec | Missing |
|---|---------|------|---------|
| 1 | FutureTools (Matt Wolfe) | Matt Wolfe | email |
| 2 | There's An AI For That | (Founder) | email |
| 3 | AI Tool Hunt | (Founder) | email; LinkedIn |
| 4 | Toolify.ai | (Founder) | email; LinkedIn |
| 5 | Ben's Bites | Ben Tossell | email; LinkedIn |
| 6 | The Rundown AI | (Founder) | email; LinkedIn |
| 7 | Superhuman AI | (Founder) | email; LinkedIn |

---

## Confidence Levels
- **H (High):** Verified from a public company page this session (HTTP 200)
- **M (Medium):** Likely correct based on company conventions; not independently confirmed from a public page
- **L (Low):** Best available; needs owner LLM verification

---

## Company Blocks (DMPS order)

### FutureTools (Matt Wolfe) — futuretools.io

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Matt Wolfe — Founder | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Personal LinkedIn(s)** | https://www.linkedin.com/in/catherine-r-mcmillan, https://www.linkedin.com/in/davidheuff, https://www.linkedin.com/in/matt-wolfe-30841712, https://www.linkedin.com/in/roya-lotfi-06609a17b | H |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of futuretools.io (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 85 | — |
| **Intent signal** | AI tools directory; 'Top AI Guys' brand | — |
| **Note** | — | — |

### There's An AI For That — theresanaiforthat.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | (Founder) — Founder | — |
| **Company LinkedIn** | https://www.linkedin.com/company/taaft | H |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of theresanaiforthat.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 82 | — |
| **Intent signal** | AI tool aggregator | — |
| **Note** | — | — |

### AI Tool Hunt — aitoolhunt.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | (Founder) — Founder | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of aitoolhunt.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 80 | — |
| **Intent signal** | AI tool discovery | — |
| **Note** | site did not respond to sweep (blocked/timeout/JS) — verify via LinkedIn search (L) | — |

### Toolify.ai — toolify.ai

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | (Founder) — Founder | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of toolify.ai (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 78 | — |
| **Intent signal** | AI tools directory | — |
| **Note** | no public contact found on sweep | — |

### Ben's Bites — bensbites.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | Ben Tossell — Founder | — |
| **Company LinkedIn** | MISSING — not found on site sweep | — |
| **Public email** | **MISSING** — no public address found on site sweep | — |
| **Source** | live curl sweep of bensbites.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 75 | — |
| **Intent signal** | AI newsletter | — |
| **Note** | no public contact found on sweep | — |

### The Rundown AI — therundown.ai

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | (Founder) — Founder | — |
| **Company LinkedIn** | MISSING — site not yet swept (verify via owner LLM) | — |
| **Public email** | **MISSING** — site not yet swept (verify via owner LLM) | — |
| **Source** | live curl sweep of therundown.ai (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 72 | — |
| **Intent signal** | AI newsletter | — |
| **Note** | site did not respond to sweep within session window — exec email/LinkedIn must be resolved via LinkedIn search (L) | — |

### Superhuman AI — superhumanai.com

| Field | Detail | Confidence |
|-------|--------|:---:|
| **Exec** | (Founder) — Founder | — |
| **Company LinkedIn** | MISSING — site not yet swept (verify via owner LLM) | — |
| **Public email** | **MISSING** — site not yet swept (verify via owner LLM) | — |
| **Source** | live curl sweep of superhumanai.com (+/about /contact /team) 2026-08-15; leads file for exec/DMPS | — |
| **DMPS** | 70 | — |
| **Intent signal** | AI community | — |
| **Note** | site did not respond to sweep within session window — exec email/LinkedIn must be resolved via LinkedIn search (L) | — |

---

## Coverage Summary

| Metric | Count |
|---|---|
| Companies covered | 7 |
| Companies with verified public email (H) | 0 |
| Companies with verified LinkedIn (H) | 2 |
| Companies with NO public exec email | 7 — see §0 |

---

## Notes on Method & Caveats
- **No emails invented.** Every direct exec email above is either (a) publicly listed on the company site (H), or (b) carried from the leads file / format-inferred and marked M/L. Any M/L address must be verified before sending (owner LLM or hunter.io-style lookup).
- **LinkedIn URLs marked H** come from the company's own site footer/about pages — these are the companies' own published links, so risk is low; LinkedIn itself was not logged into, so profile pages weren't fetched directly.
- **Exec names/roles** are carried from the leads file at M confidence (they were researched previously); only company-level contact data was re-verified live this session.
- **JS-heavy / bot-protected sites** returned no extractable contacts — for these, exec email is MISSING by design and LinkedIn should be resolved via LinkedIn search (marked L).

## Files
- **Lead list:** `leads_*` in `topaiguys.com/01-research/`
- **Verification sweep:** `/home/team/shared/contact-verification/batch5-findings-2026-08-15.txt` (raw live-sweep output)
- **Reference format:** `../nofail.ai/01-research/contact_nofail.ai.md`
---

## Owner External Research Reconciliation (2026-08-31)

> **Source:** `docs/EXEC-CONTACT-RESEARCH_2026-08-31.md` (owner-ran external research agent; received 2026-08-31).
> **Provenance note:** These LinkedIn URLs / exec names / emails come from the owner's external agent and are **candidate-H until team spot-validates**. They were NOT re-verified live by this researcher (the repo's own on-site H data from prior sweeps remains authoritative where present). Exec names are resolved where the sheet previously said "(unnamed)".
> **Email confidence:** ✅ `H` = literally published (fireable after spot-check) · ⚠️ `L` = aggregator-sourced (ContactOut/Clay), NOT literally published — LinkedIn DM recommended, disclose provenance.
> **Turnover flags** note execs **no longer in role** — do NOT pitch to them as current decision-makers.

### Resolved contacts (per domain)

| Company | Exec | Current Title | LinkedIn (owner-candidate) | Email | Email Conf | Notes |
|---|---|---|---|---|---|---|
| FutureTools (Matt Wolfe) | Matt Wolfe | Founder | `/in/matt-wolfe-30841712` | `MISSING` | — | — |
