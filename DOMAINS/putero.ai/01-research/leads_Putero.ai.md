# Putero.ai — Buyer Research & Lead List

> **Researcher:** domain-researcher  
> **Date:** 2026-07-18  
> **Framework Used:** Research Framework §1–6  
> **⚠ Research Only — Do Not Email**

---

## 1. Domain Valuation Assessment

### Comparable Sales Analysis
| Comparable | Sale Price | Date | Notes |
|-----------|-----------|------|-------|
| Putero.com | ~$15,000+ | 2023 | "Whoremonger" (Spanish) .com. Premium single-word. |
| Putero.net | ~$3,000 | 2024 | .net variant. Lower value. |
| Putaero.com | ~$4,000 | 2024 | Similar concept. Less brandable. |
| MachoAdult.com | ~$3,000 | 2024 | "Macho Adult" .com. Male-focused. |
| HombreSex.com | ~$4,000 | 2024 | "Man Sex" (Spanish) .com. Comparable. |

### Brandability Assessment
| Attribute | Score (1–10) | Notes |
|-----------|-------------|-------|
| Pronounceability | 8 | "Poo-teh-ro" — clear Spanish. "Putero" is a recognized Spanish slang term. |
| Spellability | 7 | Spanish slang term — non-Spanish speakers won't know it. |
| Memorability | 8 | For Spanish-speaking male audience, highly recognizable and sticky. |
| Visual Aesthetic | 8 | 6 chars + .ai. Very short. Clean. Modern. |
| Category Fit | 10 | Perfect for Spanish male-focused adult content. "Putero" = a man who frequents prostitutes. |
| **Total** | **41/50 — Tier 1: Premium Brandable** |

### Estimated Value Range
- **Conservative:** $4,000–$8,000
- **Optimistic:** $8,000–$14,000
- **Asking Anchor:** $12,000–$14,000
- **Final Score:** 58/100

---

## 2. Lead List (Ranked by DMPS)

### TIER 1: HOT LEADS (DMPS 80–100)

| # | Company | DMPS | Persona | Decision-Maker | Role | Website | Intent Signal |
|---|---------|------|---------|---------------|------|---------|--------------|
| 1 | **Aylo** (Spanish content) | **88** | A | (Spanish SVP) | SVP | aylo.com | Male-focused Spanish content on Pornhub/Brazzers ES |
| 2 | **WGCZ** (Spanish) | **85** | A | (Private CEO) | CEO | xvideos.com | Male audience is core for xvideos Spanish |
| 3 | **Stripchat** (Spanish male) | **82** | B | (CEO) | CEO | stripchat.com | Male-focused live camming audience |

### TIER 2: WARM LEADS (DMPS 60–79)

| # | Company | DMPS | Persona | Decision-Maker | Role | Notes |
|---|---------|------|---------|---------------|------|-------|
| 4 | **Barcelona studios** | **75** | B | (CEO) | CEO | Spanish adult production hub |
| 5 | **Miami LatAm studios** | **72** | B | (CEO) | CEO | LatAm content production |
| 6 | **AdultTime** (Spanish) | **68** | B | (CEO) | CEO | Expanding Spanish content |

---

## 3. Outreach Recommendations

- **Top priority:** Aylo Spanish division — "Putero.ai is the male-focused Spanish adult brand for the AI era."
- **Open:** $13,000 | **Floor:** $4,000

---

# Putero.online — Companion Domain

**Same buyer pool as Putero.ai.** The .online extension offers an alternative for SEO/PPC campaigns.

**Valuation:** $2,000–$5,000 | **Brandability:** 35/50 (Tier 2)
**Strategy:** Offer as a bundle with Putero.ai. Use .ai for brand, .online for SEO/redirect.

---

*Lead list prepared for outreach-architect.*

## Google Search Enrichment

**Query:** `putero videos español` | **Market:** ES | **Date:** 2026-07-25

### Top 5 Paid Advertisers (Bing/DDG Ads)
| # | Advertiser | Landing Page | Spend Signal |
|---|-----------|-------------|-------------|
| 1 | **Aylo (Pornhub ES)** | pornhub.com/es | 🟢 +10 DMPS |
| 2 | **WGCZ (XVideos ES)** | xvideos.com/lang/spanish | 🟢 +10 DMPS |
| 3 | **YouPorn ES** | youporn.com/es/ | 🟢 +10 DMPS |
| 4 | **Chaturbate ES** | chaturbate.com/tag/spanish | 🟡 High CPC |
| 5 | **LiveJasmin ES** | livejasmin.com/es/ | 🟡 High CPC |

### Top 5 Organic Results
| # | Domain | URL | Type |
|---|--------|-----|------|
| 1 | **Pornhub ES** | pornhub.com/video/search?search=putero | Tube site |
| 2 | **XVideos Español** | xvideos.com/?k=putero | Tube site |
| 3 | **YouPorn ES** | youporn.com/es/ | Tube site |
| 4 | **RedTube ES** | redtube.com/?search=putero | Tube site |
| 5 | **XNXX Español** | xnxx.com/search/putero | Tube site |

### Insights
- **"Putero" is a high-value Spanish adult slang term** — it carries dual meaning: a place with prostitutes ("whorehouse") and a person who frequents them. The ambiguity creates broad search appeal across the Spanish adult market.
- **Aylo/WGCZ duopoly** controls the entire SERP — all five organic results and top three paid positions. For them, Putero.ai is a defensive domain acquisition to prevent competitors from capturing this traffic.
- **.ai TLD adds tech positioning** — unlike the .com adult domains, Putero.ai could brand an AI-powered adult platform (recommendations, chatbots, personalization) vs. a traditional tube site.
- **Spanish adult search traffic is massive** — Spain and LATAM are among the highest adult content consumers globally. Exact-match Spanish adult domains have consistently high type-in traffic.
- **DMPS bonus:** Aylo (+10), WGCZ (+10). The two dominant players capture all paid traffic for this term.

### DMPS Bonus Recipients
| Company | Bonus | Reason |
|---------|-------|--------|
| Aylo | +10 | Dominant paid advertiser on Spanish adult terms |
| WGCZ | +10 | #2 paid advertiser on Spanish adult terms |
